...
func (c *Capture) Observe(event events.Envelope) {
	...
	case events.EventMessage:
		...
		} else if message := payloadString(event.Payload, "message", ""); message != "" {
			c.messageDeltas++
	...
	case events.EventError:
		c.lastError = payloadString(event.Payload, "message", payloadString(event.Payload, "error", c.lastError))
		if status := payloadString(event.Payload, "status", ""); status != "" && c.finishStatus == "" {
			c.finishStatus = status
		}
	case events.EventToolStarted:
		c.toolStarted++
		name := payloadString(event.Payload, "tool_name", payloadString(event.Payload, "display_name", "tool"))
	...
	case events.EventToolFailed:
		c.toolFailed++
		name := payloadString(event.Payload, "tool_name", "tool")
		errText := payloadString(event.Payload, "error", payloadString(event.Payload, "message", "failed"))
	...
	case events.EventFinish:
		c.finishStatus = payloadString(event.Payload, "status", c.finishStatus)
		c.finishMessage = payloadString(event.Payload, "message", c.finishMessage)
		if c.lastError == "" {
			c.lastError = payloadString(event.Payload, "error", "")
		}
		if text := payloadString(event.Payload, "text", ""); text != "" && c.message.Len() == 0 {
			c.message.WriteString(text)
		}
	}
}
...
func (c *Capture) FailureError(headline string) error {
	...
	parts := []string{
		headline,
		fmt.Sprintf("subagent=%s", c.options.Name),
