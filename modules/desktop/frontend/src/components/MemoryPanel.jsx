import { ArrowLeft, Database, Eye, Plus, RefreshCw, Save, Trash2 } from 'lucide-react';
...
function previewText(item) {
  const text = String(item?.content || item?.title || '').trim().replace(/\s+/g, ' ');
  if (!text) return '无内容';
  return text.length > 96 ? `${text.slice(0, 96)}…` : text;
}

function emptyListMessage({ loading, workspaceRoot, currentSessionId, scopeFilter }) {
  ...
  if (!currentSessionId && (scopeFilter === 'all' || scopeFilter === 'session')) {
    return { title: '未选择会话', detail: '选择会话后可查看会话记忆。' };
  }
  return { title: '暂无记忆记录', detail: '新建一条事实、偏好或决策，供后续运行注入。' };
}
...
export function MemoryPanel({ apiJson, currentSessionId, workspaceRoot }) {
  ...
  if (mode === 'edit') {
    return (
      <section className="memory-panel-content is-edit" data-testid="memory-panel" data-mode="edit">
        <div className="memory-edit-header">
          <IconButton data-testid="memory-back" label="返回列表" onClick={backToList}>
            <ArrowLeft size={15} />
          </IconButton>
          <div className="memory-edit-heading">
            <strong>{selected ? '编辑记忆' : '新建记忆'}</strong>
            <span>{selected ? compactTime(selected.updatedAt) : '填写后保存，可在后续运行中注入'}</span>
          </div>
        </div>
  ...
  return (
    <section className="memory-panel-content is-list" data-testid="memory-panel" data-mode="list">
      <PanelHeader
        action={(
          <div className="memory-header-actions">
            ...
            <Button data-testid="memory-create" icon={<Plus size={14} />} onClick={startCreate} variant="soft">
              新建
            </Button>
          </div>
        )}
      ...
      <div className="memory-list" data-testid="memory-list">
        {items.length === 0 ? (
          <EmptyState title={emptyCopy.title}>
            {emptyCopy.detail}
          </EmptyState>
        ) : (
          items.map((item) => (
            <button
              className={classNames('memory-item', item.id === selectedId && 'active')}
              data-testid="memory-item"
              key={item.id}
              onClick={() => selectMemory(item)}
              type="button"
            >
              <span className={classNames('memory-kind-dot', `is-${item.kind || 'fact'}`)} aria-hidden="true">
                <Database size={13} />
              </span>
      ...
      <div className="memory-preview">
        ...
        {previewOpen && preview ? (
          ...
          <InlineEmpty className="activity-empty">预览当前会注入 Provider 的记忆上下文。</InlineEmpty>
        )}
      </div>
