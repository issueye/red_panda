...
- MCP **tools/call execution MVP (docs/36 D2)**: Gateway injects enabled MCP server configs into `agent.reply` options (`mcp_servers`); Runtime discovers tools (`mcp__server__tool`), applies global policy/permission, executes one-shot stdio `tools/call` with start/initialize/call/shutdown timeouts, and cleans up processes. Default MCP tool risk is high; per-server allowlist/risk overrides apply.
