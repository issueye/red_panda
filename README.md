...
3. Agent Runtime owns provider calls, tool execution, permission requests, cancellation, and Worker pool execution.
...
- Skill context boundary: the catalog is injected at conversation start; the model reads a matching `.codex/skills/<name>/SKILL.md` with `workspace.read_file` and decides how to apply it. Delegated coding Workers receive the built-in coding skill automatically. Worker messages remain persisted for Desktop/audit display but are excluded before the 200-message root conversation limit.
...
- MCP execution boundary: Agent Runtime spawns each enabled MCP server as a stdio child process on demand and drives the full MCP lifecycle — `initialize`, `tools/list`, and `tools/call`. Discovered tools are registered as provider-facing tools under canonical names (`mcp__server__tool`), routed through the existing ToolRunner, and default to `RiskHigh` so they honor `tool_policy` / `tool_allowlist` / `tool_denylist` / `permission_mode`. MCP tool calls emit the standard `tool_started` / `tool_output` / `tool_finished` / `tool_failed` events; each call is a one-shot stdio session with start/initialize/list/call/shutdown timeouts, continuous bounded (8KB) stderr drain, and secret redaction in diagnostics. Per-server raw tool allowlists and risk overrides apply. A crash/restart budget auto-disables any server that hits 3 startup-phase failures (spawn / open / initialize) within a 60s window (override via `RED_PANDA_MCP_CRASH_LIMIT` / `RED_PANDA_MCP_CRASH_WINDOW_MS`; `=0` disables); business `tools/call` failures are not counted. MCP process reuse, cross-run discovery cache, and protocol-level `notifications/cancelled` (on run cancel / call timeout) are implemented.
...
- Runtime memory tools: `memory.list`, `memory.create`, `memory.update`, and `memory.delete` are exposed to the provider/tool loop. Runtime keeps tool policy, permission, and tool events; Gateway executes persistence through internal `state.tool.execute` (domain=memory) JSON-RPC and validates session/workspace scope.
...
Gateway manages MCP server configuration records through `/api/v1/mcp/servers`. The Desktop MCP Settings tab uses these APIs for list, create, edit, enable/disable, delete, discovery (`POST .../discover`), and try-call (`POST .../call` with `tool_name` + JSON arguments). Try-call runs a management-path `tools/call` without starting a chat run; conversation-time MCP calls still go through the provider tool loop. Configuration includes command argv, environment entries, working directory, normalized phase timeouts, raw tool allowlists, and risk overrides. Sensitive environment values are masked in API responses.
...
- MCP process reuse, discovery cache, crash/restart budget, protocol-level cancellation, and Desktop Settings try-call UI are implemented.
