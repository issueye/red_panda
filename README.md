...
- Remaining MCP work is performance and robustness hardening: per-server process reuse across calls, cross-run discovery cache, and MCP protocol-level cancellation notifications. The crash/restart budget (auto-disable servers that repeatedly fail to start) is implemented.
